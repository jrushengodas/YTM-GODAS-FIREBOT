...
exports.run = async (runRequest) => {
    ...
    try {
        ...
        if (!input) {
            await unlock(vars);
            await setVar(vars, "ytm_sr_last_message_godas", "❌ Aucun titre ou lien fourni pour le Skip Prio.");
            return true;
        }
        ...
        if (!sent) {
            await unlock(vars);
            await setVar(vars, "ytm_sr_last_message_godas", "❌ Impossible d'ajouter le Skip Prio dans YTM.");
            return true;
        }

        logger.info("SKIP PRIO | Attente insertion YTM...");
        await sleep(1800);
        ...
        if (!skipped) {
            await unlock(vars);
            await setVar(vars, "ytm_sr_last_message_godas", "❌ Impossible de skip la musique actuelle.");
            return true;
        }
        ...
        await addLocalHistory(vars, videoId, title, user, durationSeconds, "reward_skipprio");

        await unlock(vars);

        await setVar(vars, "ytm_sr_last_message_godas", `🔥 ${user} a utilisé un Skip Prio : ${title}`);

        return true;

    } catch (err) {
        logger.error("Erreur SkipPrio V3 : " + err.stack);

        await unlock(vars);
        await setVar(vars, "ytm_sr_last_message_godas", "❌ Erreur Skip Prio.");
...
function getApiKeys(config) {
    ...
    if (Array.isArray(config.youtubeApiKeys)) {
        return config.youtubeApiKeys.map(key => key.toString().trim()).filter(Boolean);
    }
...
function getInput(runRequest) {
    ...
    input = input.replace(/^!skipprio\s*/i, "").trim();
...
async function takeBackWaitingSR(vars, host, port, logger) {
    ...
    if (removed) {
        await addBackToLocalQueue(vars, waitingId, waitingTitle, waitingUser, waitingPriority, true);
        logger.info("SKIP PRIO | SR waiting reprise en local : " + waitingTitle);
    }
...
async function deleteQueuedVideoById(host, port, videoId, logger) {
    try {
        ...
        if (!queue) {
            logger.info("DeleteQueuedVideoById | Queue YTM illisible");
            return false;
        }
...
async function addBackToLocalQueue(vars, videoId, title, user, priority, wasAlreadyWaiting) {
...
async function skipCurrentSong(host, port, logger) {
    ...
    for (const endpoint of endpoints) {
        ...
        try {
            ...
            logger.info("Skip OK via : " + endpoint);
            return true;

        } catch (err) {
            logger.info("Skip FAIL via " + endpoint + " : " + err.message);
        }
    }
